...
# 6-row pixel font banner spelling FLAGHUNTER, striped in a rainbow gradient
# so it actually pops in a terminal instead of just being a wall of text.
_BANNER_ROWS = [
    "█████ █      ███   ████ █   █ █   █ █   █ █████ █████ ████ ",
    "█     █     █   █ █     █   █ █   █ ██  █   █   █     █   █",
    "████  █     █████ █ ███ █████ █   █ █ █ █   █   ████  ████ ",
...
_BANNER_COLORS = [C.R, C.Y, C.G, C.CY, C.B, C.M]


def print_banner():
    print()
    for row, color in zip(_BANNER_ROWS, _BANNER_COLORS):
        print(f"{color}{C.BOLD}{row}{C.END}")
    print(f"\n{C.CY}{C.BOLD}  >> Static file forensics for CTFs{C.END}"
          f"{C.END}  {C.M}|{C.END}  {C.Y}hidden data · steganography · malware indicators{C.END}")
    print(f"  {C.B}v{VERSION}{C.END}  {C.M}·{C.END}  built for CTF players 🚩\n")
...
def print_usage_guide():
    print(f"""
{C.BOLD}USAGE{C.END}
  {C.G}python3 flaghunter.py{C.END} {C.CY}<file>{C.END} [options]

{C.BOLD}ARGUMENTS{C.END}
  {C.CY}<file>{C.END}              Path to the file you want analyzed (any type)

{C.BOLD}OPTIONS{C.END}
  {C.Y}--json{C.END}              Output machine-readable JSON instead of a text report
  {C.Y}--deep{C.END}              Run deeper / slower analysis passes
  {C.Y}-h, --help{C.END}          Show this help message and exit

{C.BOLD}EXAMPLES{C.END}
  {C.G}$ python3 flaghunter.py{C.END} suspicious_image.png
      {C.CY}→ full colored text report: hashes, entropy, LSB stego, flags{C.END}

  {C.G}$ python3 flaghunter.py{C.END} challenge.pdf {C.Y}--json{C.END}
      {C.CY}→ JSON output, ready to pipe into jq / another script{C.END}

  {C.G}$ python3 flaghunter.py{C.END} mystery.bin {C.Y}--json{C.END} | jq {C.M}'.flags'{C.END}
      {C.CY}→ just the flagged findings, extracted with jq{C.END}

{C.BOLD}SEVERITY LEGEND{C.END}
  {C.R}[!! MALWARE]{C.END}     indicator commonly tied to malicious files
  {C.M}[HIDDEN DATA]{C.END}    concrete hidden data found
  {C.Y}[SUSPICIOUS]{C.END}     worth a human look
  {C.CY}[info]{C.END}           general notes, no action needed
""")
    print(f"{C.BOLD}{'─'*72}{C.END}\n")


class ColorHelpParser(argparse.ArgumentParser):
    """argparse subclass that shows the banner + a neat custom help screen."""
    ...
    def error(self, message):
        print_banner()
        print(f"{C.R}{C.BOLD}Error:{C.END} {message}\n")
        print_usage_guide()
        sys.exit(2)
...
def main():
    ...
    if not os.path.isfile(args.file):
        print_banner()
        print(f"{C.R}{C.BOLD}Error:{C.END} file not found: {args.file}\n", file=sys.stderr)
        sys.exit(1)

    # Skip the banner in --json mode so piped/scripted output stays clean.
    if not args.json:
        print_banner()
        print(f"{C.CY}▸ scanning:{C.END} {C.BOLD}{args.file}{C.END} ...")
...
if __name__ == "__main__":
    try:
        ...
        # happens harmlessly when piping output into `head`, `jq`, etc.
        try:
            sys.stdout.close()
        except Exception:
            pass
